// gcc twoBodyInBox3.c -o temp2 -lglut -lm -lGLU -lGL
...
{
	float dx,dy,dz,r,r2,dvx,dvy,dvz;
	for (int i = 0; i < NUMBER_OF_SPHERES; i++)
	{
		for (int j = 0; j < NUMBER_OF_SPHERES; j++)
		{
			if (j == i)
				...
				r = sqrt(r2);
			if (r <= DIAMETER) // if less than 2 diameters apart, that means they have collided 
			{
				...
				// -------- Option 1 ----------
				SpheresCPU[i].vx = - SpheresCPU[i].vx*0.9;
				SpheresCPU[i].vy = - SpheresCPU[i].vy*0.9;
				SpheresCPU[i].vz = - SpheresCPU[i].vz*0.9;

				SpheresCPU[j].vx = - SpheresCPU[j].vx*0.9;
				SpheresCPU[j].vy = - SpheresCPU[j].vy*0.9;
				SpheresCPU[j].vz = - SpheresCPU[j].vz*0.9;
				// -------- Option 2 -----------
				/*float temp = vx[i];
				vx[i] += vx[j];
				vx[j] = temp;
				...
				SpheresCPU[i].px += DT*SpheresCPU[i].px;
				SpheresCPU[i].py += DT*SpheresCPU[i].py;
				SpheresCPU[i].pz += DT*SpheresCPU[i].pz;
				
				SpheresCPU[j].px += DT*SpheresCPU[j].px;
				SpheresCPU[j].py += DT*SpheresCPU[j].py;
				SpheresCPU[j].pz += DT*SpheresCPU[j].pz;
			}
		}
	}
}
		...
		for (int j = i; j < NUMBER_OF_SPHERES; j++)
		{
			if(j!=i)
			{
				...
				if (r < DIAMETER)
				{
					...
					SpheresCPU[j].r= 1.0*rand()/RAND_MAX;
					SpheresCPU[j].g= 0.5*rand()/RAND_MAX;
					SpheresCPU[j].b= 1.0*rand()/RAND_MAX;
