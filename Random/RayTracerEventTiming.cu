...
{	
	if(key == 'q')
	{
		...
		printf("\nw Good Bye\n");
		exit(0);nvcc RayTracerAnimation.cu -o Regular -lglut -lGL -lm
...
__device__ float hit(float pixelx, float pixely, float *dimingValue, sphereStruct sphere)
{
	...
	if(dx*dx + dy*dy < r2) // if the ray nvcc RayTracerAnimation.cu -o Regular -lglut -lGL -lmhits the sphere, then we need to find distance
	{
