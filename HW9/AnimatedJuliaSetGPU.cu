...
float RealSeed = 0.276387625;
float ImaginarySeed = -0.4953556789211;
...
{	
	...
	if(key == 'r') //reset
	{
		RealSeed = 0.276387625;
		ImaginarySeed = -0.4953556789211;
		DeltaSeed = 0.01;
	}
}
...
__global__ void colorPixels(float *pixels, float xMin, float yMin, float dx, float dy, float a, float b) 
{
	...
	{
		pixels[id]   = 0.2; //Setting the red
		pixels[id+1] = 0.7; //Setting the green
