...
float *A0_GPU, *B0_GPU, *C0_GPU, *A1_GPU, *B1_GPU, *C1_GPU,*A2_GPU, *B2_GPU, *C2_GPU,*A3_GPU, *B3_GPU, *C3_GPU; //GPU pointers
...
{	
	...
	cudaMalloc(&A3_GPU,DATA_CHUNKS*sizeof(float));
	myCudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&B3_GPU,DATA_CHUNKS*sizeof(float));
	myCudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&C3_GPU,DATA_CHUNKS*sizeof(float));
	myCudaErrorCheck(__FILE__, __LINE__);
...
int main()
{
	...
	for(int i = 0; i < ENTIRE_DATA_SET; i += DATA_CHUNKS*4)
	{
		//******************************************
		...
		myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(A3_GPU, A_CPU+i+3*DATA_CHUNKS, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream3);
		myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(B3_GPU, B_CPU+i+3*DATA_CHUNKS, DATA_CHUNKS*sizeof(float), cudaMemcpyHostToDevice, Stream3);
		myCudaErrorCheck(__FILE__, __LINE__);
		...
		trigAdditionGPU<<<GridSize,BlockSize,0,Stream3>>>(A3_GPU, B3_GPU, C3_GPU, DATA_CHUNKS);
		...
		myCudaErrorCheck(__FILE__, __LINE__);
		cudaMemcpyAsync(C_CPU+i+3*DATA_CHUNKS, C3_GPU, DATA_CHUNKS*sizeof(float), cudaMemcpyDeviceToHost, Stream3);
		myCudaErrorCheck(__FILE__, __LINE__);
		//******************************************
	}
