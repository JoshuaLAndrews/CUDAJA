...
int main()
{
	...
	myCudaErrorCheck(__FILE__, __LINE__);
	cudaMemcpyAsync(C_GPU, C_CPU, N*sizeof(float), cudaMemcpyHostToDevice);
	myCudaErrorCheck(__FILE__, __LINE__);
	
/*	for (int i=655000;i<656000;i++)
	{
	printf("Temp = %.15f  k=%d\n", C_CPU[i],i);
	}
*/	
...
//	cudaMemcpyAsync(C_CPU, C_GPU, 656000*sizeof(float), cudaMemcpyDeviceToHost);
//	myCudaErrorCheck(__FILE__, __LINE__);
